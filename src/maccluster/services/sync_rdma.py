...
def run_rdma_transfer(
    ...
    *timeout* falls back to ``TIMEOUT_SYNC`` — arep must never run unbounded.
    ...
    # be transferred anyway (sync F10). Silently skipped, not an error. Anything
    # that is not a str stays in so ``_check_rel`` refuses it loudly.
    rels = [r for r in rels if not (isinstance(r, str) and is_denied_rel(r))]
    if not rels:
        return 0
